...
int numCommonLinks(const unordered_set<string> &curr_set,
  ...
  // (void) target_set;
  // (void) curr_set;
  // count_if takes in a range and a predicate, and returns the number of
  // elements in the range that satisfy the predicate.
  int count = std::count_if(curr_set.begin(), curr_set.end(),
                            [&target_set](const string &link) {
                              return target_set.find(link) != target_set.end();
                            });
  return count;
}
